...
const commands = {
	'çal': (msg) => {
		if (queue[msg.guild.id] === undefined) return msg.channel.sendMessage(`${ayarlar.prefix}çal <link> ile birkaç müzik ekle`);
		...
		(function play(song) {
			...
			collector.on('message', m => {
				if (m.content.startsWith(ayarlar.prefix + 'duraklat')) {
				...
				} else if (m.content.startsWith(ayarlar.prefix + 'atla')){
